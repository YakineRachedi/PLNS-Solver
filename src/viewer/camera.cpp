...
Camera::Camera(float aspect_ratio, float fov, Fov axis) {
	...
	float focal_ratio = 1.f / tan(fov * DEG2RAD * 0.5f);
...
Camera & Camera::set_orthographic(bool is_ortho) { frustum.is_ortho = is_ortho; return (*this) ;}
...
Camera & Camera::set_near(float near_plane) { frustum.near = near_plane; return (*this) ;}
Camera & Camera::set_far(float far_plane) { frustum.far = far_plane; return (*this) ;}
...
Mat4 Camera::world_to_view() const { return compose(-position, -rotation).as_matrix() ;}
Mat4 Camera::view_to_world() const { return compose(rotation, position).as_matrix() ;}
Mat4 Camera::world_to_clip() const { return compose(world_to_view(), view_to_clip()) ;}
Mat4 Camera::clip_to_world() const { return compose(clip_to_view(), view_to_world()) ;}
...
enum Visibility visibility(const Aabb & bbox, const float *pvm) {
	...
	if (clip_R || clip_L || clip_T || clip_B || clip_F || clip_N)
		return Visibility::None;
	return fully_in ? Visibility::Full : Visibility::Partial;
}